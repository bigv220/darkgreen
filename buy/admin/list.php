...
if($job=="list")
{
	...
	while($rs=$db->fetch_array($query))
	{
		...
		if(!$rs[yz]){
			...
			$rs[ischeck]="<A HREF='$admin_path&action=work&id=$rs[id]&jobs=unyz' style='color:#333333;'>已审核</A>";
		}elseif( $rs[yz]==2 ){
			$rs[ischeck]="<A HREF='$admin_path&action=work&id=$rs[id]&jobs=undel' style='color:#333333;'>作废</A>";
		}
		if(!$rs[levels]){
...
function dowork($id,$job){
	...
	elseif($job=="punish")
	{
		global $Type;
		if($Type==1){
			add_user($rsdb[uid],-abs($webdb[ErrSortMoney]),'评分');
		}elseif($Type==2){
