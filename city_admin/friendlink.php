...
while($rs = $db->fetch_array($query))
{
	...
	$rs[fname] = $s[name];
	$rs[ifshow]=$rs[ifhide]?"<A HREF='#' style='color:red;'>首页隐藏</A>":"<A HREF='#' style='color:#333333;'>首页显示</A>";
	$rs[yz]=$rs[yz]?"<a href='friendlink.php?job=list&action=setyz&yz=0&id=$rs[id]' style='color:red;'><img alt='已通过审核,点击取消审核' src='../member/images/check_yes.gif' border=0></a>":"<a href='friendlink.php?job=list&action=setyz&yz=1&id=$rs[id]' style='color:#333333;'><img alt='还没通过审核,点击通过审核' src='../member/images/check_no.gif' border=0></a>";
	if(!$rs[endtime]){
		$rs[state]='长久有效';
