...
$linkdb=array(
			...
			"未认证"=>"$admin_path&job=list&renzheng=0",
			"普通会员"=>"$admin_path&job=list&renzheng=1",
			"商务通会员"=>"$admin_path&job=list&renzheng=2",
			"VIP会员"=>"$admin_path&job=list&renzheng=3",
			);

$renzhengdb=array(
	0=>'<font color="#cccccc">普通会员</font>',
	1=>'<font color="#004600">普通会员</font>',
	2=>'<font color="#FF00FF">商务通会员</font>',
...
if($job=="list")
{
	...
	while($rs=$db->fetch_array($query))
	{
		if(!$rs[yz]){
			...
			$rs[ischeck]="<A HREF='$admin_path&action=work&uid=$rs[uid]&jobs=unyz' style='color:#333333;'>已审核</A>";
		}
		if(!$rs[levels]){
