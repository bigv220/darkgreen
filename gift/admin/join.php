...
while($rs = $db->fetch_array($query))
{
	...
	$rs[yzshow]=$rs[yz]?"<A HREF='$admin_path&job=yz&id=$rs[id]&yz=0' style='color:red;'>已发放</A>":"<A HREF='$admin_path&job=yz&id=$rs[id]&yz=1' style='color:#333333;'  onclick=\"return confirm('你确定要通过审核,给他发放礼品吗?将要扣除他的{$webdb[MoneyName]}{$rs[C][money]}{$webdb[MoneyDW]}');\">未发放</A>";
	$listdb[]=$rs;
}
