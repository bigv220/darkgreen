...
if($job=="list")
{
	...
	$query = $db->query("SELECT SQL_CALC_FOUND_ROWS A.*,B.title AS company FROM {$_pre}content A LEFT JOIN {$pre}hy_company B ON A.uid=B.uid ORDER BY A.id DESC LIMIT $min,$rows");
