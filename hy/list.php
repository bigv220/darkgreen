...
// 将所有子类的产品也显示出来!
$subfid_query=$db->query("SELECT A.* FROM {$_pre}sort A WHERE A.fup='$fid'");
$fid_arr = array();
while($rs = $db->fetch_array($subfid_query)) {
    $fid_arr[] = $rs['fid'];
    $double_subfid_query=$db->query("SELECT A.* FROM {$_pre}sort A WHERE A.fup='". $rs['fid']."'");
    while($inside_rs = $db->fetch_array($double_subfid_query)) {
        $fid_arr[] = $inside_rs['fid'];
    }
}
...
if($Fid_db[0][$fid]){
    //AND A.city_id='$city_id'  需不需要多城市?
	$SQL="SELECT SQL_CALC_FOUND_ROWS DISTINCT A.* FROM {$_pre}company A LEFT JOIN {$_pre}company_fid B ON A.uid=B.uid LEFT JOIN {$_pre}sort S ON B.fid=S.fid WHERE S.fid in ($fid_str) AND A.yz=1 AND A.city_id='$city_id' ORDER BY A.rid DESC LIMIT $min,$rows";
}else{
	$SQL="SELECT SQL_CALC_FOUND_ROWS DISTINCT A.* FROM {$_pre}company A LEFT JOIN {$_pre}company_fid B ON A.uid=B.uid WHERE B.fid in ($fid_str) AND A.city_id='$city_id' AND A.yz=1 ORDER BY A.rid DESC LIMIT $min,$rows";
}
