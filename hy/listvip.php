...
if($Fid_db[0][$fid]){
	$SQL="SELECT SQL_CALC_FOUND_ROWS DISTINCT A.* FROM {$_pre}company A LEFT JOIN {$_pre}company_fid B ON A.uid=B.uid LEFT JOIN {$_pre}sort S ON B.fid=S.fid WHERE S.fup='$fid' AND A.city_id='$city_id' AND A.yz=1 AND A.renzheng>1 ORDER BY A.rid DESC LIMIT $min,$rows";
}else{
	$SQL="SELECT SQL_CALC_FOUND_ROWS DISTINCT A.* FROM {$_pre}company A LEFT JOIN {$_pre}company_fid B ON A.uid=B.uid WHERE B.fid='$fid' AND A.city_id='$city_id' AND A.yz=1 AND A.renzheng>1 ORDER BY A.rid DESC LIMIT $min,$rows";
}
...
$showpage=getpage("","","listvip.php?fid=$fid",$rows,$totalNum);
