...
function getpage($table,$choose,$url,$rows=20,$total=''){
	...
	$showpage="<a href=\"$url&page=1\" title=\"首页\">首页</A> <a href=\"$url&page=$uppage\" title=\"上一页\">上一页</A>  {$show}  <a href=\"$url&page=$nextpage\" title=\"下一页\">下一页</A> <a href=\"$url&page=$totalpage\" title=\"尾页\">尾页</A> ";
    if($totalpage>1){
		return $showpage;
	}
}
...
function sms_send($mob,$content){
	...
	if($webdb[fetion_id]&&$webdb[fetion_pwd]&&in_array($mob,explode("\r\n",$webdb[fetion_friend]))){
		...
		$url="http://sms.eshang8.com/api/?esname=$webdb[sms_es_name]&key=$webdb[sms_es_key]&msg=$content&phone=$mob&smskind=1";
