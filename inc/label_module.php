...
if(!$haveCache){
	...
	while( $rs=$db->fetch_array($query) ){
		...
		if( $rs[typesystem] )
		{
            ...
            //自定义方法！
            $_array['arr_fid'] = array();
            if ($rs[tag] ==='life_t3') {
                $hy_fid1 = array();
                // 加上城市判断
                $query1 = $db->query("SELECT * FROM `{$pre}hy_company_fid` as hcf LEFT JOIN `{$pre}hy_company` as hc
                    on hcf.uid=hc.uid WHERE hc.city_id=$city_id");
                while($hy_company1 = $db->fetch_array($query1)){
                    $hy_fid1[$hy_company1['fid']]=$hy_company1;
                }
                // 读取子类和父类的id，并为即将所做对应做准备
                $query2 = $db->query("SELECT * FROM `{$pre}hy_sort` WHERE fup!=0");
                while($hy_sort1 = $db->fetch_array($query2)){
                    if (is_array($hy_fid1[$hy_sort1['fid']])) {
                        $hy_fid1[$hy_sort1['fup']]=$hy_sort1['fid'];
                    }
                }
                $_array['arr_fid'] = $hy_fid1;
            }

            if ($rs[tag] ==='life_t5') {
                $shop_fid = array();
                $query_s = $db->query("SELECT * FROM `{$pre}shop_content` WHERE city_id=$city_id");
                while($dianpu_company = $db->fetch_array($query_s)){
                    $shop_fid[$dianpu_company['fid']]=$dianpu_company;
                }
                $query_ss = $db->query("SELECT * FROM `{$pre}shop_sort` WHERE fup!=0");
                while($shop_sort = $db->fetch_array($query_ss)){
                    if (is_array($shop_fid[$shop_sort['fid']])) {
                        $shop_fid[$shop_sort['fup']]=$shop_sort['fid'];
                    }
                }
                $_array['arr_fid'] = $shop_fid;
            }
            if ($rs[tag] ==='life_t6') {
                $news_fid = array();
                $query_news = $db->query("SELECT * FROM `{$pre}news_content` WHERE city_id=$city_id");
                while($news = $db->fetch_array($query_news)){
                    $news_fid[$news['fid']]= $news;
                }
                $query_n = $db->query("SELECT * FROM `{$pre}news_sort` WHERE fup!=0");
                while($news_sort = $db->fetch_array($query_n)){
                    if (is_array($news_fid[$news_sort['fid']])) {
                        $news_fid[$news_sort['fup']]=$news_sort['fid'];
                    }
                }
                $_array['arr_fid'] = $news_fid;
            }
            if ($rs[tag] ==='life_t7') {
                $dianpu_fid = array();
                // 加上城市判断
                $query2 = $db->query("SELECT * FROM `{$pre}dianpu_company_fid` as hcf LEFT JOIN `{$pre}dianpu_company` as hc
                    on hcf.uid=hc.uid WHERE hc.city_id=$city_id");
                while($dianpu_company = $db->fetch_array($query2)){
                    $dianpu_fid[$dianpu_company['fid']]=$dianpu_company;
                }
                $query3 = $db->query("SELECT * FROM `{$pre}dianpu_sort` WHERE fup!=0");
                while($hy_sort = $db->fetch_array($query3)){
                    if (is_array($dianpu_fid[$hy_sort['fid']])) {
                        $dianpu_fid[$hy_sort['fup']]=$hy_sort['fid'];
                    }
                }
                $_array['arr_fid'] = $dianpu_fid;
            }
            $_array['label'] = $rs[tag];
