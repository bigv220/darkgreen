...
function select_fupmenu($name='fid',$id=0){
	...
	while($rs = $db->fetch_array($query)){
		...
		$select.="<option value='$rs[id]' $ckk style='color:#333333;'>$rs[name]</option>";
	}
	$select.="</select>";
	return $select;
}