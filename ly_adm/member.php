...
if($job=="list"&&$Apower[member_list])
{
	...
	while($rs=$db->fetch_array($query)){
		...
		if($rs[yz]){
			...
			$rs[yz]="<A HREF='index.php?lfj=$lfj&action=yz&uid_db[0]=$rs[uid]&T=yz' style='color:#333333;' onclick=\"$rs[alert]\" title='还没有通过审核,点击即可通过审核'><img src='../member/images/check_no.gif' border='0'></A>";
		}else{
			$rs[yz]="未激活";
		}
