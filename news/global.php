...
function ListThisSort2($rows,$leng){
	global $page,$fid,$fidDB,$SQL,$city_id;
	if($page<1){
		$page=1;
	}
	...
	$min=($page-1)*$rows;
	$_SQL="WHERE A.fid in($fid".GetAllSonFid($fid).") AND A.yz=1 AND city_id='$city_id' $SQL ORDER BY $sql_list $sql_order LIMIT $min,$rows";
	$listdb=list_content($_SQL,$leng);
	return $listdb;
}
...
function ListOnlySort($rows){
	...
	while($rs=$db->fetch_array($query))
	{
		...
		if($rs[type]){
			...
			if($fiddb){
				$fids=implode(",",$fiddb);
				$_SQL="WHERE A.fid IN ($fids) AND A.yz=1 AND A.city_id = {$city_id} ORDER BY $_SonOrder DESC LIMIT $_SonRow";
			}
		}else{
			
			$_SQL="WHERE A.fid=$rs[fid] AND A.yz=1 AND A.city_id = {$city_id} ORDER BY $_SonOrder DESC LIMIT $_SonRow";
		}
		if($_SQL){
...
function fake_html($content){
	...
	$content=preg_replace("/list\.php\?fid=([\d]+)&page=([\d]+)/eis","fake_html_Function('$listpath','\\1','','\\2','')",$content);
	$content=preg_replace("/buy\.php\?fid=([\d]+)&page=([\d]+)/eis","fake_html_Function('$listpath','\\1','','\\2','')",$content);	//有分页的列表页
