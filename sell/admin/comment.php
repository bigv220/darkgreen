...
if($job=="list")
{
	...
	while($rs=$db->fetch_array($query)){
		...
		if($rs[yz]==1){
			$rs[yz]="<A HREF='$admin_path&action=list&jobs=unyz&ciddb[{$rs[cid]}]=$rs[cid]' style='color:#333333;'>已审核</A>";
		}elseif($rs[yz]==0){
	...
	elseif($jobs=="good"||$jobs=="ungood")
	{
		foreach($ciddb AS $key=>$rs){
			...
			if($jobs=="good"&&$rs[type]!=1){
				...
				add_user($rs[uid],abs($webdb[GoodCommentMoney]),'优秀留言奖励');
			}elseif($jobs=="ungood"&&$rs[type]==1){
				$db->query(" UPDATE {$_pre}comments SET type='0' WHERE cid='$key' ");
				add_user($rs[uid],-abs($webdb[GoodCommentMoney]),'无聊留言扣分');
			}			
			$ck++;
		}
	}
