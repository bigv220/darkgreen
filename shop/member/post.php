...
if($action=="postnew")
{
    ...
    //是否可编辑
    if($postdb[if_changed] == "on") {
        $postdb["if_changeable"] = 1;
    } else {
        $postdb["if_changeable"] = 0;
    }

    if(empty($postdb["huiyuan_price"]) ||  $postdb["huiyuan_price"] < 0 || $postdb["is_huiyuan"] <= 0)
    {
        $postdb["huiyuan_price"] = 0;
    }


	/*往标题表插入内容*/
	$db->query("INSERT INTO `{$_pre}content` (`title` , `mid` ,`fid` , `fname` ,  `posttime` , `list` , `uid` , `username` ,  `picurl` , `ispic` , `yz` , `keywords` , `ip` , `money` , `city_id`,`picnum`,`price`,`sellertext`,`if_changeable`
    ,`pinpai`,`xinghao`  ,`chicun`  ,`yanse`  ,`goucheng`  , `caizhi` , `zhongliang` , `anquan` , `zhiliang` , `biaozhun`, `gongyi`, `peijian`, `jishucaishu`, `qita`, `huiyuan_price`)
	VALUES (
	'$postdb[title]','$fidDB[mid]','$fid','$fidDB[name]','$timestamp','$postdb[list]','$lfjdb[uid]','$lfjdb[username]','$postdb[picurl]','$postdb[ispic]','$postdb[yz]','$postdb[keywords]','$onlineip','$postdb[money]','$city_id','$num','$postdb[price]','$postdb[sellertext]','$postdb[if_changeable]'
	, '$postdb[pinpai]', '$postdb[xinghao]','$postdb[chicun]','$postdb[yanse]','$postdb[goucheng]','$postdb[caizhi]','$postdb[zhongliang]','$postdb[anquan]','$postdb[zhiliang]','$postdb[biaozhun]','$postdb[gongyi]','$postdb[peijian]','$postdb[jishucaishu]','$postdb[qita]', '$postdb[huiyuan_price]')");
...
elseif($action=="edit")
{
	...
	/*更新主信息表内容*/
	$db->query("UPDATE `{$_pre}content` SET title='$postdb[title]',keywords='$postdb[keywords]',picurl='$postdb[picurl]',ispic='$postdb[ispic]',picnum='$num',price='$postdb[price]',city_id='$city_id',sellertext='$postdb[sellertext]',if_changeable=$postdb[if_changeable]
	 , pinpai='$postdb[pinpai]',xinghao='$postdb[xinghao]',chicun='$postdb[chicun]',yanse='$postdb[yanse]',goucheng='$postdb[goucheng]',caizhi='$postdb[caizhi]',
	 zhongliang='$postdb[zhongliang]',anquan='$postdb[anquan]',zhiliang='$postdb[zhiliang]',biaozhun='$postdb[biaozhun]',
	 gongyi='$postdb[gongyi]',peijian='$postdb[peijian]',jishucaishu='$postdb[jishucaishu]',huiyuan_price='$postdb[huiyuan_price]',
	 qita='$postdb[qita]' WHERE id='$id'");
