...
if($action=="search")
{
	...
	else
	{
        ...
        if (isset($_GET['fid'])) {
            $fid_where = " AND A.fid=".$_GET['fid'];
        }
		$SQL="SELECT A.*,B.* FROM {$_pre}content A LEFT JOIN {$_pre}content_$mid B ON A.id=B.id WHERE A.mid='$mid' AND $field $fid_where LIKE '%$keyword%' $_sql LIMIT $min,$rows ";
	}
